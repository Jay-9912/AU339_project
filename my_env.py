...
class School(gym.Env):  # 学校类
    ...
    def is_caught_complete(self):
        keys = list(self.thief_list.keys())
        print("num of left thieves:", len(keys))
        if len(keys) == 0:
            return True
    ...
    def cal_weight(self, me, neighbor, current_time):
        ...
        weight1 = 1000 * thief_inc + 2 * time_delta - 1 * detect_need + 50 * nearby_repulse
        weight2 =                    2 * time_delta - 1 * detect_need + 50 * nearby_repulse
        weight3 = 1000 * thief_inc                  - 1 * detect_need + 50 * nearby_repulse
        weight4 = 1000 * thief_inc + 2 * time_delta                   + 50 * nearby_repulse
        weight5 = 1000 * thief_inc + 2 * time_delta - 1 * detect_need
    ...
    def cal_detect_rate(self):
        num_of_detect = 0
        total_num = 0
        for i in range(self.graph.w):
            for j in range(self.graph.l):
                if self.time_flow[i][j] != 0:
                    num_of_detect += 1
                if self.obstacle_mask[i][j] == 0:
                    total_num += 1
        print("detected_num:", num_of_detect)
        print("total_num:", total_num)
        self.detect_rate = float((num_of_detect / total_num))

    def GetInitState(self):
        if self.agent_num == 3:
            location = [264, 1156, 1123]
        if self.agent_num == 4:
            location = [165, 235, 1194, 1165]
        if self.agent_num == 5:
            location = [165, 235, 1194, 1165, 740]
        if self.agent_num == 6:
            location = [165, 235, 1194, 1165, 730, 750]
        return location
    ...
    def detect(self, iteration):
        ...
        for agent in self.agent_list:
            ...
            for node in detect_list:
                row = node // self.graph.l
                col = node % self.graph.l
    ...
    def generate_obstacle(self, num):  # 随机产生障碍物
        ...
        obstacle_list = [483, 621, 211, 811, 980, 317, 184, 136]
...
if __name__ == '__main__':
    ...
    for i in range(1000):
        ...
        if initial_num == wait_time: #or env.is_caught_complete():
            initial_num = 0
            env.cal_loss()
            print('(value 1)patrol_loss:', env.loss)
            env.cal_detect_rate()
            print('(value 2)detect_rate:', env.detect_rate)
            env.reset_table()
